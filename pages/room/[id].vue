...
const { data } = await useFetch<IApiRoomDetailResponse>('/api/room', {
  query: {
    id: route.params.id
  }
})
...
const bedType = computed(() => {
  return room.value.descriptionShort.Bed[0]
})
...
<template>
  <article>
    <div class="hero relative h-[596px]">
      <img class="absolute top-8 left-12" src="@/assets/images/index_icon/logo_white.svg" alt="logo" />
      <div class="grid h-full grid-cols-[70%,30%] grid-rows-2">
        <img
          v-for="i in 3"
          :key="i"
          class="h-full w-full cursor-pointer object-cover first:row-span-2"
    ...
    <main class="mt-12 grid grid-cols-[1fr,13%,31.9%] gap-[3.1%] px-[5.2%]">
      <section>
        ...
        <div class="mt-10 mb-16 grid grid-cols-3 gap-8 bg-[#f0f0f0] p-8">
          <figure
      ...
      <div class="text-right leading-7 tracking-[1.px]">
        <p class="text-[30px] font-light leading-10">NT.{{ room.normalDayPrice }}</p>
        <p class="text-[14px] text-[#6D7278]">平日(一~四)</p>
        <p class="mt-2 text-[16px]">NT.{{ room.holidayPrice }}</p>
        <p class="text-[14px] text-[#6D7278]">假日(五~日)</p>
      </div>
      <div>
        <TheCalendar v-model:selected="selected" />
        <RoomReservationDialog v-model:selected="selected" :room="room"></RoomReservationDialog>
      </div>
    </main>
  </article>
</template>
